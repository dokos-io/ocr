...
def validate_over_billing(doc, method):
	...
	for item in doc.items:
		if item.purchase_receipt:
			...
			total_per_ref[item.purchase_receipt] = pr.net_total - (flt(pr.per_billed) * pr.net_total)
		else:
	...
	if not (flt(min_amount, precision=precision) <= flt(doc.net_total, precision=precision) <= flt(max_amount, precision=precision)):
		message = _("This purchase invoice can not be submitted because the net total is higher than {0} or lower than {1}").format(fmt_money(max_amount, currency=doc.currency), fmt_money(min_amount, currency=doc.currency))
