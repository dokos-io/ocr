...
const items_are_not_linked_to_purchase_document = async(frm) => {
	...
	no_purchase_order = await frappe.db.get_single_value("eTransactions Settings", "no_purchase_order")
...
const create_purchase_invoice = (frm, submit=false) => {
	...
	frappe.call({
		...
		if (submit && res.message.docstatus == 0) {
			...
			const success_action = new etransactions.ui.SuccessAction(frm, __("Purchase invoice created"), "Purchase Invoice", res.message.name)
			success_action.show();
		}
	})
}
...
const create_purchase_order = (frm, transaction_date, submit=false) => {
	...
	frappe.call({
		...
		frm.reload_doc();
		const success_action = new etransactions.ui.SuccessAction(frm, __("Purchase order created"), "Purchase Order", res.message.name)
		success_action.show();
	})
}
...
etransactions.ui.SuccessAction = class SuccessAction {
